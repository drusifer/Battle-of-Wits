...
export class GameEngine {
  ...
  #currentRiddle = null;
  #roundContext = null;
  #lastClueWasCorrect = null;
  #gobletHintUsed = false;
  ...
  async startGame() {
    ...
    this.#hearts = MAX_ROUNDS;
    this.#riddleManager = new RiddleManager(this.#gameData.createRiddleDeck());
    this.#gobletManager = new GobletManager(
      this.#gameData.createAttributeDeck(),
    );
  ...
  async answerRiddle(rawInput) {
    ...
    const correct = this.#riddleManager.checkAnswer(
      this.#currentRiddle,
      rawInput,
    );
    this.#lastClueWasCorrect = correct;
    const reactionLine = this.#vizzini.react(
      correct ? "riddle:correct" : "riddle:wrong",
    );
    const clueLine = this.#vizzini.drawClue(correct);

    this.#bus.emit("riddle:answered", {
      correct,
      answer: rawInput,
      clueLine,
      reactionLine,
  ...
  async requestHint() {
    ...
    const hintLine = [encouragement, this.#currentRiddle?.hint]
      .filter(Boolean)
      .join(" ");
    const vizziniReaction = this.#vizzini.react("hint:requested");
    let clueType = null;
    if (this.#lastClueWasCorrect !== null) {
      clueType = this.#lastClueWasCorrect ? "complement" : "insult";
    }

    this.#bus.emit("hint:requested", { hintLine, vizziniReaction, clueType });
    await this.#chatUI.whenIdle();
  }

  /**
   * Called by the UI when the player requests a goblet hint (Buttercup's goblet deck).
   * No-op outside GOBLET_PHASE or if already used this phase.
   * Costs 1 heart (emits heart:spent).
   */
  async requestGobletHint() {
    if (this.#state !== STATES.GOBLET_PHASE) return;
    if (this.#gobletHintUsed) return;
    this.#gobletHintUsed = true;
    this.#hearts = Math.max(0, this.#hearts - 1);

    const hintLine = this.#buttercup.drawGobletHint();
    const vizziniReaction = this.#vizzini.react("hint:requested");

    this.#bus.emit("heart:spent", {});
    this.#bus.emit("hint:goblet-requested", { hintLine, vizziniReaction });
    await this.#chatUI.whenIdle();
  }
  ...
  async chooseGoblet(choice) {
    ...
    const outcome =
      choice === this.#roundContext.safe ? "goblet:correct" : "goblet:poisoned";
    ...
    const reactionLines = reactors.filter(
      (entry) => entry.line !== null && entry.line !== undefined,
    );

    if (outcome === "goblet:poisoned") {
      this.#hearts = Math.max(0, this.#hearts - 1);
    }
  ...
  async #startRound(round) {
    ...
    this.#round = round;
    this.#riddleCount = 0;
    this.#lastClueWasCorrect = null;
    this.#gobletHintUsed = false;
  ...
  async #enterGobletPhase() {
    ...
    const leftDesc = this.#gramps.describeGoblet(
      this.#roundContext.leftGoblet.attributes,
    );
    const rightDesc = this.#gramps.describeGoblet(
      this.#roundContext.rightGoblet.attributes,
    );
