...
export class ChatUI extends BaseSubscriber {
  ...
  #initSubscriptions(bus) {
    ...
    this.subscribe(bus, 'riddle:presented', ({ riddle }) => this.#onRiddlePresented(riddle));
    this.subscribe(bus, 'riddle:answered', payload => this.#onRiddleAnswered(payload));
    this.subscribe(bus, 'hint:requested', payload => this.#onHintRequested(payload));
    this.subscribe(bus, 'goblets:described', payload => this.#onGobletsDescribed(payload));
    this.subscribe(bus, 'goblet:chosen', payload => this.#onGobletChosen(payload));
  ...
  #onRiddlePresented(riddle) {
    this.#chainAnimated('Vizzini', `Here is your riddle: "${riddle.question}"`, 'gameplay');
  }

  /** @param {{correct: boolean, clueLine: string, reactionLine: string}} payload */
  #onRiddleAnswered(payload) {
    const { clueLine, reactionLine } = payload;
    if (reactionLine) this.#chainAnimated('Vizzini', reactionLine, 'gameplay');
    if (clueLine) this.#chainAnimated('Vizzini', clueLine, 'gameplay');
  }
  ...
  #onHintRequested(payload) {
    ...
    if (vizziniReaction) this.#chainAnimated('Vizzini', vizziniReaction, 'gameplay');
  }

  /** @param {{left: string, right: string}} payload */
  #onGobletsDescribed(payload) {
    const { left, right } = payload;
    this.#chainAnimated('Gramps', `The goblet on the left: ${left}`, 'gameplay');
    this.#chainAnimated('Gramps', `The goblet on the right: ${right}`, 'gameplay');
  }
  ...
  #chainAnimated(char, line, style) {
    this.#idlePromise = this.#idlePromise.then(() => this.#appendBubbleAnimated(char, line, style));
  }
