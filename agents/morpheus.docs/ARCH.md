...
src/
  ...
  data/
    riddles.yaml        # [{question, answer, alternates[], hint}] — 100+ entries
    attributes.yaml     # Deck-of-Decks structure — 20+ categories × 5+ variants
    conversations.yaml  # ConversationDecks + ReactionDecks + Gramps connectives
...
### Attribute Data (`data/attributes.json`)
```json
{
  "Handle": [
    {
...
GOBLET_PHASE { round }
  ...
  ──► await gramps.describeGoblets(roundContext)
...
| `goblet:chosen` | `{ choice, outcome, reactionLines[] }` |
| `game:won` | `{ rounds }` |
| `game:lost` | `{}` |
...
When hint requested:
1. Draw from `buttercupGobletDeck` (goblet attribute clue from safe variant)
2. Append `riddle.hint` text (from the current riddle's data)
3. GameEngine emits combined hint line + Vizzini's `hint:requested` reaction
...
4. `data/attributes.json` — 5 categories × 5 variants minimum (enough for a real game)
