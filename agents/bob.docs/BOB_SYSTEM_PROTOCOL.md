...
### Step 0: Orient (on first `init` or new session)
Run `make tldr` to get a fast summary of all project files and get up to speed quickly.
Run `make help` to see all available targets.

### Step 0b: Trim The Chat log
 If the CHAT.md log is more than 80,000 lines move the first 50,000 lines of teh CHAT.md log into a dated long summarize the first ~50,000 into a dense summary to replce the archved content.
...
### Step 5: Post to Chat
Update `agents/CHAT.md` with your status using `make chat`.
...
```bash
make tldr    # fast orientation — TL;DR summaries from all project files
