...
test('T-GUI-1: intro scene renders Gramps dialogue on load', async ({ page }) => {
  await page.goto('/');

  // Gramps sets the scene in the intro — at least one bubble must appear
  await waitForSpeaker(page, 'Gramps');

  const grampsBubbles = page.locator('.chat-bubble .chat-name', { hasText: 'Gramps' });
...
test('T-GUI-3: goblet CTA buttons appear when goblet phase starts', async ({ page }) => {
  await page.goto('/');
...
test('T-GUI-4: Buttercup hint contains no goblet-reveal vocabulary during riddle phase', async ({
  ...
  const buttercupBubble = page.locator('.chat-bubble', {
