...
function makeReactionDecks(outcomes) {
  return Object.fromEntries(
    outcomes.map((o) => [
      o,
      makeDeck([`${o} line 1`, `${o} line 2`, `${o} line 3`]),
    ]),
  );
}
...
describe("Gramps", () => {
  function makeConnectives() {
    return new Deck(["with", "its", "topped by", "bearing", "and"], {
      autoReshuffle: true,
    });
  }
  ...
  function makeGramps() {
    return new Gramps(
      makeReactionDecks(["goblet:correct", "goblet:poisoned"]),
      makeConnectives(),
    );
  }
  ...
  it("describeGoblet() uses connectives between fragments", () => {
    ...
    const hasConnective = ["with", "its", "topped by", "bearing", "and"].some(
      (c) => desc.includes(c),
    );
