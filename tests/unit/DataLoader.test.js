...
describe("buildGameData", () => {
  ...
  it("riddleDeck is a Deck instance", () => {
    const { riddleDeck } = buildGameData(
      rawRiddles,
      rawAttributes,
      rawConversations,
    );
  ...
  it("createAttributeDeck is a function", () => {
    const { createAttributeDeck } = buildGameData(
      rawRiddles,
      rawAttributes,
      rawConversations,
    );
  ...
  it("throws if any category has fewer than 5 variants", () => {
    const badAttributes = {
      ...rawAttributes,
      TooFew: [rawAttributes.Material[0]],
    };
    expect(() =>
      buildGameData(rawRiddles, badAttributes, rawConversations),
    ).toThrow(/TooFew.*5/i);
  });

  it("does not throw with valid data", () => {
    expect(() =>
      buildGameData(rawRiddles, rawAttributes, rawConversations),
    ).not.toThrow();
  });
  ...
  it("conversations object has expected keys", () => {
    const { conversations } = buildGameData(
      rawRiddles,
      rawAttributes,
      rawConversations,
    );
  ...
  it("grampsConnectives is a Deck", () => {
    const { grampsConnectives } = buildGameData(
      rawRiddles,
      rawAttributes,
      rawConversations,
    );
